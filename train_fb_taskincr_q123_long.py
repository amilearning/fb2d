...
def main():
    ...
    schedule = [60_000, 120_000, 180_000]
    for stage_idx in range(3):
        cum_qs = QS[:stage_idx+1]
        buf = fill_buffer(npz, cum_qs)
        n_updates = schedule[stage_idx]
        print(f"\n=== STAGE {stage_idx+1}: train on {cum_qs} (buf={buf.size}) for {n_updates} updates ===")
        t0 = time.time()
        for step in range(1, n_updates + 1):
            batch = buf.sample(args.batch_size, device=device)
            m = agent.update(batch)
            if step % 5000 == 0:
                print(f"  step {step:6d}/{n_updates}  fb={m['fb_loss']:.2f} "
