"""
train_fb_offline.py — Train ONE FB model from scratch on a fixed dataset
sampled from a chosen list of quadrants. No env interaction during training.

Usage:
  --quadrants Q1,Q2,Q3       (any subset / order)
  --steps_per_quadrant 15000 (random rollout steps to pre-collect per quadrant)
  --updates 60000            (gradient updates from the buffer)
"""
...
def collect_from_quadrant(buf, q, n_steps, seed):
    env = Nav2DQuadrant(quadrant=q, max_steps=200, seed=seed)
    obs = env.reset()
...
def main():
    ...
    p.add_argument("--quadrants", type=str, required=True, help="comma-sep, e.g. Q1,Q2")
    ...
    p.add_argument("--save_dir", type=str, default="checkpoints_offline")
    ...
    print(f"quadrants={qs}  updates={args.updates}  out={save_dir}")
    ...
    print(f"Buffer filled: {buf.size} transitions from {qs}")
