"""FDWS sweep: tau=0.5, sensitivity=current

Naive sequential FB on q123 (Q1 -> Q2 -> Q3), 60k updates/stage, z_dim=32.

  - z-sampling : FDWS (Fisher-weighted), temperature tau = 0.5
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_fdws_sweep/tau05_sensCurrent")
    ...
    # W&B init
    wandb_run = maybe_init_wandb(args, "fdws_tau05", "piGram+FBl2", "sensCurrent")
    ...
    plt.suptitle(f"FDWS tau=0.5 sens=current | seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
