#!/usr/bin/env python3
"""Launch FDWS temperature sweep: 6 configs × 3 seeds = 18 runs."""
import os, subprocess, time

HERE       = os.path.dirname(os.path.abspath(__file__))
FB2D       = os.path.dirname(HERE)
LOG_DIR    = os.path.join(HERE, "real_logs")
SAVE_ROOT  = os.path.join(FB2D, "checkpoints_fdws_sweep_real")
...
scripts = sorted(f for f in os.listdir(HERE) if f.startswith("train_fdws_") and f.endswith(".py"))
assert len(scripts) == 6, f"expected 6, got {len(scripts)}"
...
log(f"starting FDWS sweep: {total} jobs, MAX_PARALLEL={MAX_PARALLEL}")
...
while True:
    while len(procs) < MAX_PARALLEL and not exhausted:
        try:
            ...
            exhausted = True
            break
        name = fname.replace("train_fdws_", "").replace(".py", "")
        run_name = f"fdws_{name}_seed{seed}"
