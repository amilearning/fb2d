#!/usr/bin/env python3
"""Generator for 6 FDWS parameter-sweep scripts.

Grid:  temperature tau in {0.5, 1.0, 5.0}  x  sensitivity data source in {current, replay}

All scripts share:
  - FDWS z-sampling (monkey-patched agent.sample_z)
  - Sensitivity source = FB  (through M = F * B^T)
  - Distillation = piGram(replay) + FB_l2(replay)
  - alpha_distill = 1.0
  - EMA = 0.0 (replace, no accumulation)
  - Final eval only
"""
...
TAUS = [0.5, 1.0, 5.0]
SENS_SOURCES = ["current", "replay"]

# tau value -> filename-safe string (no dot)
TAU_LABEL = {0.5: "05", 1.0: "10", 5.0: "50"}
# sens source -> CamelCase for filename
SENS_LABEL = {"current": "Current", "replay": "Replay"}


# ---------------------------------------------------------------------------
# Sensitivity computation block — differs by source
# ---------------------------------------------------------------------------

SENS_CURRENT = '''\
        # Compute z-sensitivity on CURRENT stage's data (EMA=0.0 -> replace)
        o_t = torch.from_numpy(npz[f"{{cur_q}}_obs"]).float().to(device)
        a_t = torch.from_numpy(npz[f"{{cur_q}}_act"]).float().to(device)
        sensitivity = compute_z_sensitivity(agent, teacher, o_t, a_t,
                                             source="FB", n_samples=1024)
        print(f"    sensitivity computed on {{cur_q}} data (current), "
...
TEMPLATE = '''\
"""FDWS sweep: tau={tau}, sensitivity={sens_source}

Naive sequential FB on q123 (Q1 -> Q2 -> Q3), 60k updates/stage, z_dim=32.

  - z-sampling : FDWS (Fisher-weighted), temperature tau = {tau}
  - Sensitivity: computed through M = F * B^T on {sens_source} data
...
def main():
    ...
    # W&B init
    wandb_run = maybe_init_wandb(args, "fdws_tau{tau_label}", "piGram+FBl2", "sens{sens_label}")
...
{SENS_BLOCK}
    ...
    plt.suptitle(f"FDWS tau={tau} sens={sens_source} | seed{{args.seed}}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
...
def render(tau, sens_source):
    tau_label  = TAU_LABEL[tau]
    sens_label = SENS_LABEL[sens_source]
    save_dir   = f"checkpoints_fdws_sweep/tau{tau_label}_sens{sens_label}"

    if sens_source == "current":
        sens_block = SENS_CURRENT
    else:
        sens_block = SENS_REPLAY
    ...
    repls = [
        ...
        ("{tau_label}",    tau_label),
        ("{sens_source}",  sens_source),
        ("{sens_label}",   sens_label),
        ("{SAVE_DIR_DEFAULT}", save_dir),
        ("{SENS_BLOCK}",   sens_block),
    ]
    for k, v in repls:
        out = out.replace(k, v)
    # Unescape double-braces for the final Python f-strings
...
def main():
    n = 0
    for tau in TAUS:
        for sens in SENS_SOURCES:
            src = render(tau, sens)
            tau_label  = TAU_LABEL[tau]
            sens_label = SENS_LABEL[sens]
            fname = f"train_fdws_tau{tau_label}_sens{sens_label}.py"
            path = os.path.join(HERE, fname)
            with open(path, "w") as f:
                f.write(src)
            print(f"  wrote {fname}")
            n += 1
    print(f"\nwrote {n} scripts to {HERE}")
