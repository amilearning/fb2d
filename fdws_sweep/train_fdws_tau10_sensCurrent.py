"""FDWS sweep: tau=1.0, sensitivity=current

Naive sequential FB on q123 (Q1 -> Q2 -> Q3), 60k updates/stage, z_dim=32.

  - z-sampling : FDWS (Fisher-weighted), temperature tau = 1.0
  - Sensitivity: computed through M = F * B^T on current data
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_fdws_sweep/tau10_sensCurrent")
    ...
    # W&B init
    wandb_run = maybe_init_wandb(args, "fdws_tau10", "piGram+FBl2", "sensCurrent")
    ...
    plt.suptitle(f"FDWS tau=1.0 sens=current | seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
