"""vMF K=20 (FIXED) + FDWS sensitivity routing + FB+pi distillation.

z-sampling: sample from a 20-component vMF mixture (frozen mu's & kappa's),
then apply FDWS per-dim weighting: z_raw ~ vMF_mixture -> z_j = z_raw_j * exp(-tau * sigma_j)
-> normalize to sphere of radius sqrt(z_dim).
...
# ---------------------------------------------------------------------------
# vMF mixture sampling (non-differentiable / fixed)
# ---------------------------------------------------------------------------

def init_vmf_params(K, z_dim, seed):
    """Initialize K vMF components with random mu's and kappa's (frozen)."""
    ...
    # kappa uniform in [5, 15]
    kappa_np = rng.uniform(5.0, 15.0, size=(K,)).astype(np.float32)
    mus = torch.from_numpy(mu_np).to(device)       # (K, z_dim)
    kappas = torch.from_numpy(kappa_np).to(device)  # (K,)
    return mus, kappas


def sample_vmf_mixture(mus, kappas, n):
    """Sample n z-vectors from a uniform mixture of K vMF components.

    Uses the tangent-normal approximation: z = mu_k/||mu_k|| + noise/sqrt(kappa_k),
    then normalize and scale to sqrt(z_dim).
    """
    K, z_dim = mus.shape
    # Uniform component assignment
    assignments = torch.randint(0, K, (n,), device=mus.device)
    mu_sel = mus[assignments]          # (n, z_dim)
    kappa_sel = kappas[assignments]    # (n,)

    mu_norm = Fn.normalize(mu_sel, dim=-1)  # (n, z_dim)
    noise = torch.randn(n, z_dim, device=mus.device) / kappa_sel.unsqueeze(-1).sqrt()
    z = mu_norm + noise
    z = Fn.normalize(z, dim=-1) * math.sqrt(z_dim)
    return z


def make_vmf_fdws_sampler(mus, kappas, sensitivity, tau):
    """Return a sampling function: n -> (n, z_dim) on sphere."""
    def _sample(n):
        z_raw = sample_vmf_mixture(mus, kappas, n)
        # FDWS per-dim weighting
        weights = torch.exp(-tau * sensitivity)
        weights = weights / weights.mean()
        z = z_raw * weights.unsqueeze(0)
        z = Fn.normalize(z, dim=-1) * math.sqrt(mus.shape[1])
        return z
    return _sample
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_vmf20_fdws/fixed")
    ...
    # Initialize FROZEN vMF params
    mus, kappas = init_vmf_params(args.vmf_K, args.z_dim, args.seed)
    print(f"vMF K={args.vmf_K}, z_dim={args.z_dim}")
    print(f"  mu norms: {mus.norm(dim=-1).cpu().numpy()}")
    print(f"  kappas:   {kappas.cpu().numpy()}")

    # W&B init
    wandb_run = maybe_init_wandb(args, "vmf20_fdws_fixed", "piGram+FBl2", "D7")
    ...
    for stage_idx in range(3):
        ...
        # Monkey-patch sample_z
        if sensitivity is not None:
            sampler_fn = make_vmf_fdws_sampler(mus, kappas, sensitivity,
                                                args.fdws_temperature)
            agent.sample_z = sampler_fn
        else:
            # Stage 0: plain vMF mixture (no sensitivity yet)
            agent.sample_z = lambda n: sample_vmf_mixture(mus, kappas, n)
    ...
    plt.suptitle(f"vMF20_FDWS_fixed | seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
