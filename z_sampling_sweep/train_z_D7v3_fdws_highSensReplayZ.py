"""D7v3: FDWS + distill, replay distillation uses HIGH-sensitivity z.

Training z ~ FDWS (low sensitivity → free dims for new task).
Distillation z ~ inverse-FDWS (high sensitivity → anchor old task where it matters).

This targets the distillation at z-regions that old tasks actually use,
rather than wasting gradient on irrelevant z-regions.
"""
...
from distill_lib import (make_teacher, sample_z, joint_update, get_F, get_B, get_pi, loss_l2, loss_gram, SAReplayBuffer, maybe_init_wandb, wandb_log, wandb_finish)
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_z_sampling/D7v3_fdws_highSensReplayZ")
    ...
    # W&B init
    wandb_run = maybe_init_wandb(args, "fdws", "piGram+FBl2", "D7")
        ...
        for step in range(1, args.updates_per_stage + 1):
            ...
            if teacher is None:
                ...
                def fb_extra_fn(ctx):
                    ...
                    # HIGH-sensitivity z for replay distillation
                    z_r      = fdws_sample_z_high_sens(
                                   s_r.size(0), agent.z_dim, _sens,
                                   temperature=args.fdws_temperature,
                                   device=device)
    ...
    plt.suptitle(f"D7 | fdws_B | seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
