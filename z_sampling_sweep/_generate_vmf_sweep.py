#!/usr/bin/env python3
"""Generator for vMF component-count sweep: 9 scripts.

vMF mixture (non-task-binding) × {B+pi, F+pi, FB+pi} distill (replay) × {5, 10, 20} components.
"""
import os
...
N_COMPS = [5, 10, 20]
# (label, X_target, fb_closure_current_unused, fb_closure_replay)
DISTILL_VARIANTS = [
    ("Bpi", "B", '''\
                def fb_extra_fn(ctx):
...
TEMPLATE = '''"""vMF mixture (K={K}) + {distill_label} distill (piGram + {X}_l2, replay)

Non-task-binding: z sampled from uniform mixture of K={K} vMF components.
Distillation: piGram (actor, replay) + {X}_l2 (FB side, replay).
"""
...
from z_sampling_lib import VMFTaskSampler
...
def main():
    ...
    p.add_argument("--vmf_kappa", type=float, default=10.0)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--save_dir", default="checkpoints_vmf_sweep/vmfMix{K}_{distill_label}_replay")
    ...
    wandb_run = maybe_init_wandb(args, "vmfMix{K}+{distill_label}", "l2+gram", "replay")

    # vMF sampler with K={K} components
    vmf_sampler = VMFTaskSampler(n_tasks={K}, z_dim=args.z_dim,
    ...
    # Monkey-patch z-sampling: always use mixture of all K components
    ...
    for stage_idx in range(3):
        ...
        print(f"\\n=== STAGE {{stage_idx + 1}}: train ONLY on {{cur_q}} (buf={{buf.size}}), "
              f"teacher={{'YES' if teacher is not None else 'NO'}}, K={K} vMF mix ===", flush=True)
        t0 = time.time()
...
{PI_CLOSURE}\
    ...
    # Restore original z-sampling for eval
    ...
    for ax, mat, title, fmt, cmap in [
        ...
        for jj in range(3):
            ax.text(jj, 0, fmt.format(mat[0, jj]), ha="center", va="center",
                    fontsize=11, fontweight="bold")
        plt.colorbar(im, ax=ax, shrink=0.8)
        ax.set_title(title, fontweight="bold")
    plt.suptitle(f"vMF mix K={K} + {distill_label}(replay) — seed{{args.seed}}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
...
def render(K, distill_label, X, fb_closure):
    out = TEMPLATE
    repls = [
        ("{K}",             str(K)),
...
def main():
    n = 0
    for K in N_COMPS:
        for distill_label, X, fb_closure in DISTILL_VARIANTS:
            src = render(K, distill_label, X, fb_closure)
            fname = f"train_z_vmfMix{K}_{distill_label}_replay.py"
