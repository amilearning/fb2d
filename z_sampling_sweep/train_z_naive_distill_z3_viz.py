"""Naive z + distill z3-viz: Uniform z-sampling + piGram+FB_l2 replay distillation,
z_dim=3 for 3D sphere visualisation.

Baseline comparison for D7v3: same distillation but NO FDWS z-routing.
After each training stage, infer z-vectors for a grid of goals and visualise.
"""
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_z_sampling/naive_distill_z3_viz")
    ...
    wandb_run = maybe_init_wandb(args, "naive", "piGram+FBl2", "naive_distill_z3viz")
    ...
    for stage_idx in range(3):
        ...
        # No FDWS — use default uniform z-sampling (agent.sample_z unchanged)
        ...
        # No sensitivity computation needed (naive z-sampling)
        ...
        # z inference uses the default agent.infer_z_from_rewards (not sample_z)
        for q in QS:
