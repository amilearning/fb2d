"""vMF mixture (K=10) + Bpi distill (piGram + B_l2, replay)

Non-task-binding: z sampled from uniform mixture of K=10 vMF components.
Distillation: piGram (actor, replay) + B_l2 (FB side, replay).
"""
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_vmf_sweep/vmfMix10_Bpi_replay")
    ...
    wandb_run = maybe_init_wandb(args, "vmfMix10+Bpi", "l2+gram", "replay")
    ...
    plt.suptitle(f"vMF mix K=10 + Bpi(replay) — seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
