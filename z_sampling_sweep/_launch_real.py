#!/usr/bin/env python3
"""Launch the z-sampling sweep: 24 configs × 3 seeds = 72 runs."""
import os, subprocess, time

HERE      = os.path.dirname(os.path.abspath(__file__))
FB2D      = os.path.dirname(HERE)
LOG_DIR   = os.path.join(HERE, "real_logs")
SAVE_ROOT = os.path.join(FB2D, "checkpoints_z_sampling_real")
...
scripts = sorted(f for f in os.listdir(HERE) if f.startswith("train_z_") and f.endswith(".py"))
...
log(f"starting z-sampling sweep: {total} jobs, MAX_PARALLEL={MAX_PARALLEL}")
