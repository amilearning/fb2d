"""D7v2: FDWS + distill, but replay distillation also uses FDWS z (not uniform).

Same as D7_fdws_B except: when computing the FB and pi distillation losses on
replay data, z_r is sampled via FDWS(sigma, tau) instead of Uniform(sphere).
This ensures the distillation anchors representations in the same z-subspace
that the new task is trained in.
"""
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_z_sampling/D7v2_fdws_fdwsReplayZ")
        ...
        for step in range(1, args.updates_per_stage + 1):
            ...
            if teacher is None:
                ...
                def fb_extra_fn(ctx):
                    ...
                    # Use FDWS z for replay (not uniform) — key difference from D7
                    z_r      = fdws_sample_z(s_r.size(0), agent.z_dim, _sens,
                ...
                def pi_extra_fn(ctx):
                    ...
                    # Use FDWS z for replay (not uniform)
                    z_r    = fdws_sample_z(s_r.size(0), agent.z_dim, _sens,
                                           temperature=args.fdws_temperature,
                                           device=device)
