"""D7v3 z3-viz: FDWS + high-sens replay distill with z_dim=3 for 3D sphere visualisation.

After each training stage, infer z-vectors for a grid of goals in each quadrant
and save them.  At the end, produce a 1x3 figure (one panel per stage) with all
three quadrants' z-clouds on the unit sphere (radius sqrt(z_dim)).
"""
...
from distill_lib import (make_teacher, sample_z, joint_update, get_F, get_B, get_pi,
                          loss_l2, loss_gram, SAReplayBuffer, maybe_init_wandb,
                          wandb_log, wandb_finish)
from z_sampling_lib import compute_z_sensitivity, fdws_sample_z, fdws_sample_z_high_sens

QS = ["Q1", "Q2", "Q3"]
Q_COLORS = {"Q1": "tab:blue", "Q2": "tab:orange", "Q3": "tab:green"}
...
# ── z-inference for visualisation ────────────────────────────────────────
def collect_z_for_quadrant(agent, q, z_dim, grid_n=10, margin=0.05):
    """Return (goals [N,2], zs [N, z_dim]) for a grid of goals inside *q*."""
    (xlo, xhi), (ylo, yhi) = QUADRANT_BOUNDS[q]
    xs = np.linspace(xlo + margin, xhi - margin, grid_n).astype(np.float32)
    ys = np.linspace(ylo + margin, yhi - margin, grid_n).astype(np.float32)
    goals = np.array(np.meshgrid(xs, ys)).T.reshape(-1, 2)  # (grid_n^2, 2)

    # fixed observation samples for z inference
    ...
    return goals, np.stack(zs, axis=0)


# ── sphere wireframe ────────────────────────────────────────────────────
def draw_sphere_wireframe(ax, radius, n=30, alpha=0.08):
    u = np.linspace(0, 2 * np.pi, n)
    v = np.linspace(0, np.pi, n)
    x = radius * np.outer(np.cos(u), np.sin(v))
    y = radius * np.outer(np.sin(u), np.sin(v))
    z = radius * np.outer(np.ones_like(u), np.cos(v))
    ax.plot_wireframe(x, y, z, color="grey", linewidth=0.3, alpha=alpha)


# ── 1x3 figure (one col per stage, all 3 quadrants coloured) ────────────
def make_viz(z_data, z_dim, save_path):
    """z_data: dict  {f"S{s}_Q{q}_zs": np.array, ...}"""
    radius = math.sqrt(z_dim)
    ...
    for col, stage in enumerate(stages):
        ...
        for q in QS:
            ...
            ax.scatter(zs[:, 0], zs[:, 1], zs[:, 2],
                       c=Q_COLORS[q], label=q, s=18, alpha=0.7, edgecolors="none")
        ax.set_title(f"After {stage}", fontsize=13, fontweight="bold")
        ax.set_xlabel("z0"); ax.set_ylabel("z1"); ax.set_zlabel("z2")
        lim = radius * 1.15
        ax.set_xlim(-lim, lim); ax.set_ylim(-lim, lim); ax.set_zlim(-lim, lim)
        ax.legend(loc="upper left", fontsize=9)
    plt.suptitle(f"z-vectors on sphere (r={radius:.2f})  --  Q1=blue  Q2=orange  Q3=green",
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_z_sampling/D7v3_z3_viz")
    ...
    p.add_argument("--grid_n", type=int, default=10,
                   help="grid resolution per axis for z-viz goals")
    args = p.parse_args()
    ...
    wandb_run = maybe_init_wandb(args, "fdws", "piGram+FBl2", "D7v3_z3viz")
    ...
    # Accumulate z-viz data across stages
    z_viz = {}
    ...
    for stage_idx in range(3):
        ...
        for step in range(1, args.updates_per_stage + 1):
            ...
            if teacher is None:
                ...
                def fb_extra_fn(ctx):
                    ...
                    z_r = fdws_sample_z_high_sens(
                        s_r.size(0), agent.z_dim, _sens,
                        temperature=args.fdws_temperature, device=device)
        ...
        # ── post-stage: teacher snapshot, replay, sensitivity ────────────
        ...
        if sensitivity is None:
            ...
            sensitivity = 0.5 * sensitivity + 0.5 * new_sens
        ...
        # ── collect z-vectors for visualisation ──────────────────────────
        stage_label = f"S{stage_idx + 1}"
        print(f"  collecting z-vectors for viz ({stage_label}) ...", flush=True)
        agent.sample_z = original_sample_z  # use vanilla for inference
        for q in QS:
            goals, zs = collect_z_for_quadrant(agent, q, args.z_dim,
                                                grid_n=args.grid_n)
            z_viz[f"{stage_label}_{q}_goals"] = goals
            z_viz[f"{stage_label}_{q}_zs"] = zs
        print(f"  ... done", flush=True)

    # ── save z-viz data ──────────────────────────────────────────────────
    npz_path = os.path.join(save_dir, "z_viz_data.npz")
    np.savez(npz_path, **z_viz)
    print(f"saved {npz_path}")

    # ── generate visualisation ───────────────────────────────────────────
    viz_path = os.path.join(save_dir, "z_viz.png")
    make_viz(z_viz, args.z_dim, viz_path)
    ...
    plt.suptitle(f"D7v3 z3-viz | fdws_B | seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
