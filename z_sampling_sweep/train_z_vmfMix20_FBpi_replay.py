"""vMF mixture (K=20) + FBpi distill (piGram + FB_l2, replay)

Non-task-binding: z sampled from uniform mixture of K=20 vMF components.
Distillation: piGram (actor, replay) + FB_l2 (FB side, replay).
"""
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_vmf_sweep/vmfMix20_FBpi_replay")
    ...
    wandb_run = maybe_init_wandb(args, "vmfMix20+FBpi", "l2+gram", "replay")

    # vMF sampler with K=20 components
    vmf_sampler = VMFTaskSampler(n_tasks=20, z_dim=args.z_dim,
    ...
    for stage_idx in range(3):
        ...
        print(f"\n=== STAGE {stage_idx + 1}: train ONLY on {cur_q} (buf={buf.size}), "
              f"teacher={'YES' if teacher is not None else 'NO'}, K=20 vMF mix ===", flush=True)
        t0 = time.time()
    ...
    plt.suptitle(f"vMF mix K=20 + FBpi(replay) — seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
