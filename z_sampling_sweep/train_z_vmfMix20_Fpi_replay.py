"""vMF mixture (K=20) + Fpi distill (piGram + F_l2, replay)

Non-task-binding: z sampled from uniform mixture of K=20 vMF components.
Distillation: piGram (actor, replay) + F_l2 (FB side, replay).
"""
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_vmf_sweep/vmfMix20_Fpi_replay")
    ...
    wandb_run = maybe_init_wandb(args, "vmfMix20+Fpi", "l2+gram", "replay")
    ...
    plt.suptitle(f"vMF mix K=20 + Fpi(replay) — seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
