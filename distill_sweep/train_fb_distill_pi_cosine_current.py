"""B=pi  D=cosine  C=current

Naive sequential FB on q123 (Q1 → Q2 → Q3) with **pi-distillation** fused
into the actor loss inside `joint_update` (single combined backward pass).
  ...
  • Distillation target  : pi
  • Distillation loss    : cosine
  • Distillation input C : current
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_distill/pi_cosine_current")
    ...
    wandb_run = maybe_init_wandb(args, "pi", "cosine", "current")
        ...
        for step in range(1, args.updates_per_stage + 1):
            ...
            if teacher is None:
                ...
                def pi_extra_fn(ctx):
                    ...
                    return args.alpha_distill * loss_cosine(student, target)
                metrics = joint_update(agent, batch, pi_extra_fn=pi_extra_fn)
    ...
    plt.suptitle(f"Distill pi  |  cosine  |  current  —  seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
