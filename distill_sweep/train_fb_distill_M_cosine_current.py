"""B=M  D=cosine  C=current

Naive sequential FB on q123 (Q1 → Q2 → Q3) with **M-distillation** fused
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_distill/M_cosine_current")
    ...
    wandb_run = maybe_init_wandb(args, "M", "cosine", "current")
    ...
    plt.suptitle(f"Distill M  |  cosine  |  current  —  seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
