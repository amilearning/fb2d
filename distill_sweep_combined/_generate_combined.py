#!/usr/bin/env python3
"""Generator for the 24 combined π+X distillation scripts.

Design:
  - π distillation is ALWAYS applied (on actor loss) using replay (s,a).
    π loss ∈ {gram, l2} (2 options).
  - X distillation is applied on the FB loss.
    X target ∈ {F, B, FB, M, Q, FBM} (6 options).
    X loss = l2 (fixed).
    X input ∈ {current, replay} (2 options).

Total: 2 × 6 × 2 = 24 scripts.

Naming: train_fb_combined_pi{Gram|L2}_{X}_{Cx}.py
"""
import os

HERE = os.path.dirname(os.path.abspath(__file__))

PI_LOSSES = [("Gram", "loss_gram"), ("L2", "loss_l2")]
X_TARGETS = ["F", "B", "FB", "M", "Q", "FBM"]
CX_OPTIONS = ["current", "replay"]

# ---------------------------------------------------------------------------
# fb_extra_fn closures for each (X, Cx) pair.  X loss is always l2.
# ---------------------------------------------------------------------------
FB_CLOSURES = {
    ("F", "current"): '''\
                def fb_extra_fn(ctx):
                    student = (ctx["F1"], ctx["F2"])
                    with torch.no_grad():
                        target = get_F(teacher, ctx["obs"], ctx["act"], ctx["z"])
...
''',
    ("FB", "current"): '''\
                def fb_extra_fn(ctx):
                    with torch.no_grad():
                        Ft1, Ft2 = get_F(teacher, ctx["obs"], ctx["act"], ctx["z"])
...
''',
    ("M", "current"): '''\
                def fb_extra_fn(ctx):
                    student = ctx["F1"] @ ctx["B"].T
                    with torch.no_grad():
                        target = get_M(teacher, ctx["obs"], ctx["act"], ctx["z"])
                    return args.alpha_distill * loss_l2(student, target)
''',
    ("M", "replay"): '''\
                def fb_extra_fn(ctx):
...
''',
    ("Q", "current"): '''\
                def fb_extra_fn(ctx):
                    student = (ctx["F1"] * ctx["z"]).sum(-1)
                    with torch.no_grad():
                        target = get_Q(teacher, ctx["obs"], ctx["act"], ctx["z"])
                    return args.alpha_distill * loss_l2(student, target)
''',
    ("Q", "replay"): '''\
                def fb_extra_fn(ctx):
    ...
    ("FBM", "current"): '''\
                def fb_extra_fn(ctx):
                    with torch.no_grad():
                        ...
                        Mt       = Ft1 @ Bt.T
                    # student F + B
                    loss_fb = loss_l2((ctx["F1"], ctx["F2"], ctx["B"]),
                                      (Ft1, Ft2, Bt))
                    # student M
                    Ms = ctx["F1"] @ ctx["B"].T
                    loss_m  = loss_l2(Ms, Mt)
                    return args.alpha_distill * (loss_fb + loss_m)
''',
    ("FBM", "replay"): '''\
                def fb_extra_fn(ctx):
                    s_r, a_r = sa_replay.sample(args.batch_size)
                    ...
                    with torch.no_grad():
                        ...
                        Bt       = get_B(teacher, s_r)
                        Mt       = Ft1 @ Bt.T
                    loss_fb = loss_l2((Fs1, Fs2, Bs), (Ft1, Ft2, Bt))
                    loss_m  = loss_l2(Ms, Mt)
                    return args.alpha_distill * (loss_fb + loss_m)
''',
}
...
# ---------------------------------------------------------------------------
# pi_extra_fn closures — always uses replay
# ---------------------------------------------------------------------------
PI_CLOSURES = {
    "Gram": '''\
                def pi_extra_fn(ctx):
...
# Template
# ---------------------------------------------------------------------------
TEMPLATE = '''"""Combined: pi({pi_loss}, replay) + {X}(l2, {Cx})

Naive sequential FB on q123 with TWO simultaneous distillation losses:
  1. π distillation ({pi_loss}, replay) — added to actor_loss via pi_extra_fn
  2. {X} distillation (l2, {Cx}) — added to FB loss via fb_extra_fn
"""
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_distill_combined/pi{pi_loss}_{X}_{Cx}")
    ...
    wandb_run = maybe_init_wandb(args, "pi{pi_loss}+{X}", "l2", "{Cx}")
    ...
    for stage_idx in range(3):
        ...
        for step in range(1, args.updates_per_stage + 1):
            ...
            if teacher is None:
                ...
                # --- X distillation on FB loss ---
{FB_CLOSURE}\

                # --- π distillation on actor loss (always replay) ---
{PI_CLOSURE}\
        ...
        # add this stage's (s, a) to the shared replay buffer
    ...
    plt.suptitle(f"pi({pi_loss},replay) + {X}(l2,{Cx})  —  seed{{args.seed}}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
...
def render(pi_loss_name, pi_loss_fn, X, Cx):
    fb_closure = FB_CLOSURES[(X, Cx)]
    pi_closure = PI_CLOSURES[pi_loss_name]
    out = TEMPLATE
    repls = [
        ("{pi_loss}", pi_loss_name),
        ("{X}",       X),
        ("{Cx}",      Cx),
        ("{FB_CLOSURE}", fb_closure),
        ("{PI_CLOSURE}", pi_closure),
    ]
    for k, v in repls:
        out = out.replace(k, v)
...
def main():
    n = 0
    for pi_name, pi_fn in PI_LOSSES:
        for X in X_TARGETS:
            for Cx in CX_OPTIONS:
                src = render(pi_name, pi_fn, X, Cx)
                fname = f"train_fb_combined_pi{pi_name}_{X}_{Cx}.py"
