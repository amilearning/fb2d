"""Combined: pi(Gram, replay) + B(l2, replay)
...
Naive sequential FB on q123 with TWO simultaneous distillation losses:
  ...
  2. B distillation (l2, replay) — added to FB loss via fb_extra_fn
"""
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_distill_combined/piGram_B_replay")
    ...
    wandb_run = maybe_init_wandb(args, "piGram+B", "l2", "replay")
    ...
    plt.suptitle(f"pi(Gram,replay) + B(l2,replay)  —  seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
