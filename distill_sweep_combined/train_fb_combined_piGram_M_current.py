"""Combined: pi(Gram, replay) + M(l2, current)
...
Naive sequential FB on q123 with TWO simultaneous distillation losses:
  ...
  2. M distillation (l2, current) — added to FB loss via fb_extra_fn
"""
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_distill_combined/piGram_M_current")
    ...
    wandb_run = maybe_init_wandb(args, "piGram+M", "l2", "current")
    ...
    plt.suptitle(f"pi(Gram,replay) + M(l2,current)  —  seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
