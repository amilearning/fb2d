"""Combined: pi(L2, replay) + FBM(l2, current)

Naive sequential FB on q123 with TWO simultaneous distillation losses:
  1. π distillation (L2, replay) — added to actor_loss via pi_extra_fn
...
def main():
    ...
    p.add_argument("--save_dir", default="checkpoints_distill_combined/piL2_FBM_current")
    ...
    wandb_run = maybe_init_wandb(args, "piL2+FBM", "l2", "current")
    ...
    plt.suptitle(f"pi(L2,replay) + FBM(l2,current)  —  seed{args.seed}",
                 fontsize=11, fontweight="bold")
    plt.tight_layout()
